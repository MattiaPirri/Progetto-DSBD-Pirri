# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import sla_pb2 as sla__pb2


class SlaServiceStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.GetSLASet = channel.unary_stream(
                '/sla.SlaService/GetSLASet',
                request_serializer=sla__pb2.SLARequest.SerializeToString,
                response_deserializer=sla__pb2.SLASetResponse.FromString,
                )


class SlaServiceServicer(object):
    """Missing associated documentation comment in .proto file."""

    def GetSLASet(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_SlaServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'GetSLASet': grpc.unary_stream_rpc_method_handler(
                    servicer.GetSLASet,
                    request_deserializer=sla__pb2.SLARequest.FromString,
                    response_serializer=sla__pb2.SLASetResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'sla.SlaService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class SlaService(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def GetSLASet(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(request, target, '/sla.SlaService/GetSLASet',
            sla__pb2.SLARequest.SerializeToString,
            sla__pb2.SLASetResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
